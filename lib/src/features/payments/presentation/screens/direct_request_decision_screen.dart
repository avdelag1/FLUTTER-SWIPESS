...
class _DirectRequestDecisionScreenState
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!mounted) return;
      ref
          .read(directRequestDetailControllerProvider.notifier)
  ...
  Future<void> _respond(
    ...
    try {
      final result = await ref
          .read(directRequestActionControllerProvider.notifier)
          .respond(requestId: widget.requestId, accept: accept);
      if (!mounted) return;
      ...
      if (senderId != null && conversationId != null) {
        await showChatPopup(
          ...
          conversation: ChatConversation(
            ...
            name: 'Direct Request',
            lastMessage: request['message']?.toString() ?? '',
            timestamp: 'now',
          ),
        );
      }
  ...
  Widget build(BuildContext context) {
    ...
    final detail = ref.watch(directRequestDetailControllerProvider);
    final action = ref.watch(directRequestActionControllerProvider);
    final request = detail.value;
        ...
        child: SafeArea(
          child: detail.isLoading
              ...
              : detail.hasError
                  ? Center(
                      child: Text(
                        'Could not load this Direct Request.',
                                ...
                                children: [
                                  ...
                                  Expanded(
                                    child: ElevatedButton.icon(
                                      onPressed: busy
                                          ? null
                                          : () => _respond(true, request),
                                      icon: const Icon(
                                        Icons.chat_bubble_rounded,
                                      ),
                                      label: Text(
                                        busy ? 'OPENING…' : 'ACCEPT & CHAT',
                                      ),
                                    ),
                                  ),
                                ],
