...
///
/// Native iOS/Android keeps Flutter video_player. Web/PWA uses the browser's
/// native <video> element via the conditional export in
/// listing_video_inline_preview.dart.
...
class _ListingVideoInlinePreviewState extends State<ListingVideoInlinePreview>
  ...
  VideoPlayerController _buildController() {
    ...
    if (file != null) return VideoPlayerController.file(File(file.path));
  ...
  Future<void> _repairLoop(VideoPlayerController controller) async {
    ...
    try {
      await controller.seekTo(Duration.zero);
      if (_lifecycleActive && !_userPaused) await controller.play();
    } catch (_) {
    } finally {
  ...
  Future<void> _toggleSound() async {
    ...
    try {
      await controller.setVolume(_sessionMuted ? 0 : 1);
      if (!_userPaused && !controller.value.isPlaying) await controller.play();
    } catch (_) {}
    if (mounted) setState(() {});
  }
