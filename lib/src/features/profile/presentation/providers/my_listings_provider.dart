...
class OwnerListingsStats {
  const OwnerListingsStats({required this.total, required this.active, required this.views, required this.avgPrice, this.categories = 0});
  final int total;
  final int active;
  final int views;
...
int _activeCategoryCount(List<Listing> listings) {
  final properties = listings.where((l) => l.category == null || l.category == 'property').isNotEmpty;
  final motorcycles = listings.any((l) => l.category == 'motorcycle');
  final bicycles = listings.any((l) => l.category == 'bicycle');
  final services = listings.any((l) => l.category == 'worker' || l.category == 'services');
  final vehicles = listings.any((l) => l.category == 'vehicle');
  return [properties, motorcycles, bicycles, services, vehicles].where((v) => v).length;
}
...
final myListingsProvider = FutureProvider.family<List<Listing>, String>((ref, status) async {
  ...
  Future<List<Listing>> primary() async {
    var filter = client.from('listings').select(
    ...
    final rows = await filter.order('created_at', ascending: false).limit(100).timeout(const Duration(seconds: 6));
    return (rows as List).map((row) => Listing.fromJson(row as Map<String, dynamic>)).toList();
  }

  Future<List<Listing>> fallback() async {
    final rows = await client.from('listings').select(
    ...
    ).eq('owner_id', userId).limit(100).timeout(const Duration(seconds: 4));
    final all = (rows as List).map((row) => Listing.fromJson(row as Map<String, dynamic>)).toList();
    if (status == 'active') return all.where((l) => l.isActive == true || l.status == 'active').toList();
...
final ownerListingsStatsProvider = FutureProvider<OwnerListingsStats>((ref) async {
  ref.watch(authStateProvider);
  final all = await ref.watch(myListingsProvider('all').future);
  if (all.isEmpty) {
    return const OwnerListingsStats(total: 0, active: 0, views: 0, avgPrice: 0, categories: 0);
  }
...
class OwnerListingsActions {
  ...
  Future<void> setStatus(String id, String status) async {
    await _ref.read(listingRepositoryProvider).updateListingStatus(listingId: id, status: status);
...
final ownerListingsActionsProvider = Provider<OwnerListingsActions>((ref) => OwnerListingsActions(ref));
