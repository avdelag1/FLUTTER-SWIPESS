...
extension _KindUi on _Kind {
  ...
  String get aliases => switch (this) {
        ...
        _Kind.service => 'service services worker chef massage cleaning dining restaurant',
...
class _RealMapboxScreenV3State extends ConsumerState<RealMapboxScreenV3> {
  ...
  Point _point(double lat, double lng) => Point(coordinates: Position(lng, lat));
  ...
  double _pitch(int km) => km <= 10 ? 42 : (km <= 50 ? 30 : (km <= 250 ? 16 : 0));
  ...
  Future<void> _onLoaded() async {
    ...
    _pins?.tapEvents(onTap: (annotation) {
  ...
  Future<void> _recenter() async {
    ...
    try {
      await map.flyTo(
        CameraOptions(center: _point(_userLat!, _userLng!), zoom: 14.7, pitch: 42, bearing: 12),
  ...
  Future<Uint8List> _makePin(_Kind kind) async {
    ...
    painter.paint(canvas, ui.Offset(44 - painter.width / 2, 38 - painter.height / 2));
  ...
  ({double lat, double lng}) _spread(String key, double lat, double lng, {double meters = 100}) {
    var hash = 137;
    ...
    return (lat: lat + math.sin(angle) * latDelta, lng: lng + math.cos(angle) * lngDelta);
  }

  ({double lat, double lng}) _cityPoint(String key, String? city, DiscoveryLocation loc) {
    final cityLoc = ListingLocations.resolve(city ?? '');
    return _spread(key, cityLoc?.lat ?? loc.latitude, cityLoc?.lng ?? loc.longitude, meters: 350);
  }
  ...
  _Item _eventItem(Event event, DiscoveryLocation loc) {
    ...
    final p = _spread('event:${event.id}', resolved?.lat ?? loc.latitude, resolved?.lng ?? loc.longitude, meters: 180);
    return _Item(
      id: event.id,
      kind: _Kind.event,
      lat: p.lat,
      ...
      image: event.imageUrl ?? (event.imageUrls.isNotEmpty ? event.imageUrls.first : ''),
      price: event.price,
      event: event,
    );
  }
  ...
  bool _filterItem(_Item item) {
    ...
    final text = '${item.title} ${item.subtitle} ${item.price} ${item.kind.label} ${item.kind.aliases}'.toLowerCase();
    return q.split(RegExp(r'\s+')).where((e) => e.isNotEmpty).every(text.contains);
  }
  ...
  List<_Item> _items() {
    ...
    ].where((i) => !_hidden.contains(i.key) && _filterItem(i)).toList(growable: false);
  }
  ...
  Polygon _radiusPolygon(DiscoveryLocation loc) {
    ...
    for (var i = 0; i <= 64; i++) {
      ...
      ring.add(Position(loc.longitude + math.cos(a) * lngDelta, loc.latitude + math.sin(a) * latDelta));
    }
    return Polygon(coordinates: [ring]);
  }

  Future<void> _render() async {
    if (!_loaded || _pins == null || _userPin == null || _radius == null) return;
    final generation = ++_renderGeneration;
    final items = _items();
    final loc = ref.read(discoveryLocationProvider);
    try {
      await Future.wait([_pins!.deleteAll(), _userPin!.deleteAll(), _radius!.deleteAll()]);
      if (!mounted || generation != _renderGeneration) return;
      if (loc.radiusKm <= 250) {
        await _radius!.create(PolygonAnnotationOptions(
      ...
      if (_userLat != null && _userLng != null && _userImage != null) {
        await _userPin!.create(PointAnnotationOptions(
  ...
  Future<void> _save(_Item item) async {
    try {
      ...
      if (mounted) {
        ref.read(appNotificationsProvider.notifier).error('Could not save this yet', 'Please try again in a moment.');
      }
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    ref.listen(discoveryLocationProvider, (oldValue, next) {
      if (oldValue == null || oldValue.latitude != next.latitude || oldValue.longitude != next.longitude || oldValue.radiusKm != next.radiusKm) {
        _selected = null;
        ...
        children: [
          ...
          if (!_loaded)
            const ColoredBox(
              ...
              child: Center(child: CircularProgressIndicator(strokeWidth: 2, color: Colors.black87)),
            ),
          Positioned(
            top: 0,
            left: 0,
              ...
              child: DecoratedBox(
                decoration: BoxDecoration(
                  gradient: LinearGradient(
                    ...
                    colors: [Color(0xDFFFFFFF), Color(0x72FFFFFF), Color(0x00FFFFFF)],
                  ),
                ),
              ),
            ),
          ...
          if (_controls) ...[
            ...
            Positioned(top: pad.top + 44, left: 0, right: 0, child: _Filters(active: _filter, onTap: _setFilter)),
            if (_search)
              Positioned(
                top: pad.top + 84,
                left: 12,
            ...
            Positioned(
              top: pad.top + 7,
              right: 7,
              child: _IconOnly(icon: Icons.visibility_rounded, label: 'Show controls', onTap: () => setState(() => _controls = true)),
            ),
          Positioned(
            right: 7,
            bottom: trayHeight + pad.bottom + 11,
            child: _IconOnly(icon: Icons.my_location_rounded, label: 'My exact location', onTap: _recenter),
          ),
...
class _IconOnly extends StatelessWidget {
  const _IconOnly({required this.icon, required this.label, required this.onTap});
  final IconData icon;
  final String label;
  ...
  Widget build(BuildContext context) => Semantics(
        ...
        child: GestureDetector(
          ...
          child: SizedBox(
            width: 36,
            height: 34,
            child: Icon(icon, size: 20, color: const Color(0xFF111318), shadows: const [Shadow(color: Color(0x66FFFFFF), blurRadius: 4)]),
          ),
        ),
      );
}
              ...
              return GestureDetector(
                ...
                child: Container(
                  ...
                  child: Row(mainAxisSize: MainAxisSize.min, children: [
                    Icon(f.icon, size: 12.5, color: selected ? Colors.white : Colors.black87),
...
class _Search extends StatefulWidget {
  const _Search({required this.initial, required this.onChanged, required this.onSubmit, required this.onClear});
...
class _SearchState extends State<_Search> {
  ...
  Widget build(BuildContext context) => Container(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: const [BoxShadow(color: Color(0x23000000), blurRadius: 18, offset: Offset(0, 7))],
        ),
        child: TextField(
          ...
          style: GoogleFonts.plusJakartaSans(fontSize: 12.5, fontWeight: FontWeight.w700, decoration: TextDecoration.none),
          decoration: InputDecoration(
            border: InputBorder.none,
            hintText: 'Search property, event, yacht, moto, bike, person or city…',
            hintStyle: GoogleFonts.plusJakartaSans(color: Colors.black45, fontSize: 11.5, fontWeight: FontWeight.w600, decoration: TextDecoration.none),
...
class _Menu extends StatelessWidget {
  const _Menu({required this.trayVisible, required this.onCities, required this.onGps, required this.onTray, required this.onHide, required this.onClose});
...
class _Preview extends StatelessWidget {
  const _Preview({required this.item, required this.onOpen, required this.onSave, required this.onClose});
  final _Item item;
  ...
  Widget build(BuildContext context) => GestureDetector(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: const [BoxShadow(color: Color(0x38000000), blurRadius: 24, offset: Offset(0, 10))],
          ),
          child: Row(children: [
            ...
            Expanded(
              child: Column(mainAxisAlignment: MainAxisAlignment.center, crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(item.kind.label, style: GoogleFonts.plusJakartaSans(color: item.kind.color, fontSize: 8.5, fontWeight: FontWeight.w900, letterSpacing: .45, decoration: TextDecoration.none)),
                const SizedBox(height: 2),
                Text(item.title, maxLines: 1, overflow: TextOverflow.ellipsis, style: GoogleFonts.plusJakartaSans(fontSize: 13, fontWeight: FontWeight.w900, decoration: TextDecoration.none)),
                const SizedBox(height: 2),
                Text(item.subtitle, maxLines: 1, overflow: TextOverflow.ellipsis, style: GoogleFonts.plusJakartaSans(color: Colors.black54, fontSize: 10.5, fontWeight: FontWeight.w600, decoration: TextDecoration.none)),
              ]),
            ),
            GestureDetector(onTap: onSave, child: const SizedBox(width: 38, height: 58, child: Icon(Icons.favorite_border_rounded, color: Color(0xFFFF375F), size: 23))),
            GestureDetector(onTap: onClose, child: const SizedBox(width: 30, height: 58, child: Icon(Icons.close_rounded, color: Colors.black45, size: 17))),
          ]),
        ),
      );
}
...
class _Tray extends StatefulWidget {
  const _Tray({required this.items, required this.city, required this.selected, required this.expanded, required this.onSelect, required this.onOpen, required this.onSave, required this.onUp, required this.onDown, required this.onToggle});
          ...
          child: Column(children: [
            GestureDetector(
              ...
              child: Padding(
                ...
                child: Row(children: [
                  Expanded(child: Text('${widget.items.length} new in $city', maxLines: 1, overflow: TextOverflow.ellipsis, style: GoogleFonts.plusJakartaSans(fontSize: 11.5, fontWeight: FontWeight.w800, decoration: TextDecoration.none))),
                  Icon(widget.expanded ? Icons.keyboard_arrow_down_rounded : Icons.keyboard_arrow_up_rounded, size: 18, color: Colors.black54),
                ]),
              ),
            ),
            Container(width: 28, height: 3, decoration: BoxDecoration(color: Colors.black12, borderRadius: BorderRadius.circular(99))),
            const SizedBox(height: 5),
            Expanded(
              child: widget.items.isEmpty
                  ? Center(child: Text('Everything here is already discovered.', style: GoogleFonts.plusJakartaSans(color: Colors.black45, fontSize: 11, fontWeight: FontWeight.w600, decoration: TextDecoration.none)))
                  : ListView.separated(
                      scrollDirection: Axis.horizontal,
                      physics: const BouncingScrollPhysics(),
                      ...
                      itemBuilder: (_, i) {
                        ...
                        return _Card(
                          ...
                          onTap: () => item.key == widget.selected ? widget.onOpen(item) : widget.onSelect(item),
...
class _Card extends StatelessWidget {
  const _Card({required this.item, required this.selected, required this.expanded, required this.onTap, required this.onSave});
  final _Item item;
  final bool selected;
  final bool expanded;
  final VoidCallback onTap;
  ...
  Widget build(BuildContext context) => GestureDetector(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            boxShadow: selected ? const [BoxShadow(color: Color(0x18000000), blurRadius: 12, offset: Offset(0, 4))] : const [],
          ),
          child: ClipRRect(
            borderRadius: BorderRadius.circular(15),
            child: Stack(children: [
              Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                SizedBox(height: expanded ? 100 : 68, width: double.infinity, child: _Photo(item: item)),
                Expanded(
                  child: Padding(
                    padding: const EdgeInsets.fromLTRB(8, 6, 8, 6),
                    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text(item.title, maxLines: 1, overflow: TextOverflow.ellipsis, style: GoogleFonts.plusJakartaSans(fontSize: 10.5, fontWeight: FontWeight.w800, decoration: TextDecoration.none)),
                      const SizedBox(height: 2),
                      Text(item.subtitle, maxLines: 1, overflow: TextOverflow.ellipsis, style: GoogleFonts.plusJakartaSans(color: Colors.black45, fontSize: 8.8, fontWeight: FontWeight.w600, decoration: TextDecoration.none)),
                      if (expanded && item.price.trim().isNotEmpty) ...[
                        const Spacer(),
                        Text(item.price, maxLines: 1, overflow: TextOverflow.ellipsis, style: GoogleFonts.plusJakartaSans(fontSize: 9, fontWeight: FontWeight.w800, decoration: TextDecoration.none)),
                      ],
                    ]),
                  ),
                ),
              ...
              Positioned(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(color: item.kind.color.withValues(alpha: .92), borderRadius: BorderRadius.circular(7)),
                  child: Text(item.kind.label, style: GoogleFonts.plusJakartaSans(color: Colors.white, fontSize: 7.2, fontWeight: FontWeight.w900, letterSpacing: .35, decoration: TextDecoration.none)),
                ),
              ),
              Positioned(
                right: 3,
                ...
                child: GestureDetector(
                  onTap: onSave,
                  child: const SizedBox(width: 34, height: 34, child: Icon(Icons.favorite_border_rounded, color: Colors.white, size: 19, shadows: [Shadow(color: Colors.black54, blurRadius: 6)])),
                ),
              ),
            ]),
          ),
  ...
  Widget build(BuildContext context) {
    final child = item.image.trim().isNotEmpty
        ? Image.network(
            item.image,
            fit: BoxFit.cover,
            errorBuilder: (_, __, ___) => ColoredBox(color: const Color(0xFFF0F2F5), child: Center(child: Icon(item.kind.icon, color: item.kind.color, size: 28))),
          )
        : ColoredBox(color: const Color(0xFFF0F2F5), child: Center(child: Icon(item.kind.icon, color: item.kind.color, size: 28)));
    if (size == null) return child;
    return ClipRRect(borderRadius: BorderRadius.circular(13), child: SizedBox(width: size, height: size, child: child));
  }
}
