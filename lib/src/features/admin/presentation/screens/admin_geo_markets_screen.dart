...
class AdminGeoMarketsScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<AdminGeoMarketsScreen> createState() => _AdminGeoMarketsScreenState();
}
...
class _AdminGeoMarketsScreenState extends ConsumerState<AdminGeoMarketsScreen> {
  ...
  final _allCategories = <_GeoCategory>[
    // MOBILITY & TRANSPORT
    _GeoCategory('properties', 'Properties & Rentals', 'mobility', Icons.home_work_rounded),
    _GeoCategory('yachts', 'Yachts & Boats', 'mobility', Icons.sailing_rounded),
    _GeoCategory('exotic_cars', 'Exotic Cars', 'mobility', Icons.directions_car_rounded),
    _GeoCategory('private_jets', 'Private Jets & Helis', 'mobility', Icons.flight_takeoff_rounded),
    _GeoCategory('motorcycles', 'Motorcycles', 'mobility', Icons.two_wheeler_rounded),
    _GeoCategory('bicycles', 'Bicycles & Scooters', 'mobility', Icons.pedal_bike_rounded),
    _GeoCategory('atv_quads', 'ATVs & Quads', 'mobility', Icons.agriculture_rounded),
    _GeoCategory('golf_carts', 'Golf Carts', 'mobility', Icons.electric_car_rounded),
    _GeoCategory('chauffeurs', 'Chauffeurs & Limos', 'mobility', Icons.airport_shuttle_rounded),

    // WORKERS & PROS
    _GeoCategory('cleaners', 'Cleaning Staff', 'workers', Icons.cleaning_services_rounded),
    _GeoCategory('maintenance', 'Maintenance', 'workers', Icons.build_circle_rounded),
    _GeoCategory('security', 'Security & Bodyguards', 'workers', Icons.security_rounded),
    _GeoCategory('translators', 'Translators / Guides', 'workers', Icons.translate_rounded),
    _GeoCategory('chefs', 'Private Chefs', 'workers', Icons.restaurant_menu_rounded),
    _GeoCategory('bartenders', 'Bartenders & Mixologists', 'workers', Icons.local_bar_rounded),
    _GeoCategory('nannies', 'Nannies & Babysitters', 'workers', Icons.child_care_rounded),
    
    // BEAUTY, HEALTH & STYLE
    _GeoCategory('massage', 'Massage Therapists', 'beauty', Icons.spa_rounded),
    _GeoCategory('makeup_artists', 'Makeup Artists', 'beauty', Icons.face_retouching_natural_rounded),
    _GeoCategory('hair_stylists', 'Hair Stylists & Colorists', 'beauty', Icons.content_cut_rounded),
    _GeoCategory('personal_trainers', 'Personal Trainers', 'beauty', Icons.fitness_center_rounded),
    _GeoCategory('personal_shoppers', 'Personal Shoppers & Stylists', 'beauty', Icons.checkroom_rounded),

    // EXPERIENCES & OUTDOORS
    _GeoCategory('surf_lessons', 'Surf Instructors', 'experiences', Icons.surfing_rounded),
    _GeoCategory('scuba', 'Scuba & Freediving', 'experiences', Icons.scuba_diving_rounded),
    _GeoCategory('fishing', 'Fishing Charters', 'experiences', Icons.phishing_rounded),
    _GeoCategory('ski', 'Ski & Snowboard', 'experiences', Icons.downhill_skiing_rounded),
    _GeoCategory('desert_safari', 'Desert Safari & Camels', 'experiences', Icons.wb_sunny_rounded),
    _GeoCategory('volcano_trekking', 'Volcano / Trekking Guides', 'experiences', Icons.terrain_rounded),
    _GeoCategory('food_tours', 'Food & Culinary Tours', 'experiences', Icons.tapas_rounded),
    _GeoCategory('wine_tastings', 'Sommeliers & Wine Tastings', 'experiences', Icons.wine_bar_rounded),

    // CULTURE & ENTERTAINMENT
    _GeoCategory('djs', 'DJs & VIP Hosts', 'culture', Icons.headphones_rounded),
    _GeoCategory('photographers', 'Photographers / Paparazzi', 'culture', Icons.camera_alt_rounded),
    _GeoCategory('spiritual', 'Spiritual Guides & Cacao', 'culture', Icons.self_improvement_rounded),
    _GeoCategory('salsa', 'Salsa & Bachata Classes', 'culture', Icons.music_note_rounded),
    _GeoCategory('tango', 'Tango Classes', 'culture', Icons.music_note_rounded),
    _GeoCategory('martial_arts', 'Martial Arts / Muay Thai', 'culture', Icons.sports_martial_arts_rounded),
    _GeoCategory('kpop_dance', 'K-Pop / Dance Classes', 'culture', Icons.library_music_rounded),
  ];

  @override
  void initState() {
    super.initState();
    _selectedCity = PassportCities.all.firstWhere((c) => c.name == 'Miami', orElse: () => PassportCities.all.first);
    ...
    for (final city in PassportCities.all) {
      ...
      // Tropical / Beach Hubs
      if (['Tulum', 'Bali', 'Mykonos', 'Ibiza', 'Cabo', 'Punta Cana', 'Cancún', 'Miami', 'Sydney', 'Rio'].contains(n)) {
      ...
      if (['Dubai', 'Monaco', 'Las Vegas', 'Miami', 'LA', 'London', 'Paris', 'New York'].contains(n)) {
        _marketConfig[n]!['exotic_cars'] = true;
        _marketConfig[n]!['private_jets'] = true;
        _marketConfig[n]!['security'] = true;
      }
      
      if (['Ibiza', 'Mykonos', 'Las Vegas', 'Miami', 'Dubai', 'Tulum'].contains(n)) {
      ...
      if (['Paris', 'Rome', 'Barcelona', 'New York', 'London', 'Tokyo', 'Seoul'].contains(n)) {
                ...
                return Padding(
                  ...
                  child: ChoiceChip(
                    ...
                    shape: RoundedRectangleBorder(
                      ...
                      border: Border.all(
                        color: selected ? AppTheme.brandPrimary : ink.withAlpha(30),
                      ),
                    ),
  ...
  Widget _buildGroup(String title, List<String> groupIds, Color ink) {
    ...
    final categories = _allCategories.where((c) => groupIds.contains(c.groupId)).toList();
            ...
            children: [
              for (var i = 0; i < categories.length; i++) ...[
                ...
                if (i < categories.length - 1)
                  Divider(height: 1, indent: 56, endIndent: 16, color: ink.withAlpha(15)),
              ],
            ],
          ),
        ),
  ...
  Widget _buildCategoryTile(_GeoCategory category, Color ink) {
    ...
    return ListTile(
      leading: Container(
        ...
        decoration: BoxDecoration(
          color: isActive ? AppTheme.brandPrimary.withAlpha(20) : ink.withAlpha(10),
