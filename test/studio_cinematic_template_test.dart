...
void main() {
  group('Swipess Studio cinematic catalog', () {
    ...
    test(
      'a 3-6 photo project always resolves to a valid deterministic frame',
      () {
    ...
    test(
      'render JSON keeps focal points normalized and one shot per photo',
      () {
        final template = CinematicCatalog.templatesFor(
        ...
        final json = template.toRenderJson(
          ...
          photoFits: const <int, StudioPhotoFit>{1: StudioPhotoFit.fit},
        );
    ...
    test('Studio project keeps per-photo portrait/fit choices', () {
      ...
      expect(
        restored.photoFits[1] ?? StudioPhotoFit.portrait,
