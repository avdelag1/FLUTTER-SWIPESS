...
void main() {
  ...
  test('catalog keeps the live Cap App Store product IDs', () {
    expect(
      IapCatalog.subscriptions.map((o) => o.appleProductId).toList(),
      [
    ...
    expect(
      IapCatalog.tokens.map((o) => o.appleProductId).toList(),
      [
    ...
    expect(
      IapCatalog.eventPromos.map((o) => o.appleProductId).toList(),
      [
    ...
    expect(IapCatalog.promoById('growth')?.appleProductId,
        'Swipess.promo.event.month.v3');
  ...
  test('PayPal NCP suffixes match Cap iapProducts / AdvertisePage', () {
    expect(
      IapCatalog.subscriptions.map((o) => o.paypalPath).toList(),
      ['QSRXCJYYQ2UGY', 'HUESWJ68BRUSY', '7E6R38L33LYUJ'],
    );
    expect(
      IapCatalog.tokens.map((o) => o.paypalPath).toList(),
      ['VNM2QVBFG6TA4', 'VG2C7QMAC8N6A', '9NBGA9X3BJ5UA', 'KP9WHGEN23MYA'],
    );
    expect(
      IapCatalog.eventPromos.map((o) => o.paypalPath).toList(),
      ['ZXQC96VYV7JLL', 'ATKD4TR7KFTJU', 'LK7XWSMDHH8AW'],
    );
  });
  ...
  test('CheckoutResult copy matches Cap orchestrator outcomes', () {
    ...
    expect(
      CheckoutResult.openedWebCheckout.userMessage,
      contains('PayPal'),
    );
    expect(
      CheckoutResult.unavailable.userMessage,
      contains('App Store'),
    );
  });

  testWidgets('tokens modal lists Cap packs and PayPal web copy', (tester) async {
