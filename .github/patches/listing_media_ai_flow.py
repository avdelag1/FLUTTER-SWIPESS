...
# Swipe deck: video is always media #1 when present.
p = Path("lib/src/features/swipes/presentation/widgets/swipeable_card_stack.dart")
s = p.read_text()
s = replace_once(s, "      media.add(video);", "      media.insert(0, video);", "video first")
p.write_text(s)


# Video editor: precise start/end trim handles.
p = Path("lib/src/features/camera/presentation/screens/video_cropper_screen.dart")
s = p.read_text()
s = s.replace(
...
s = s.replace("'10s LOOP'", "'TRIM VIDEO'", 1)
old_slider = """                  SliderTheme(
...
new_slider = """                  Text(
                  ...
                  Text(
                    '${_start.toStringAsFixed(1)}s – ${_end.toStringAsFixed(1)}s',
"""
s = replace_once(s, old_slider, new_slider, "range trim")
s = s.replace(
    "'Cap looping card · max ${VideoCropperScreen.maxSeconds.toInt()}s'",
    "'Preview · trim · max ${VideoCropperScreen.maxSeconds.toInt()}s'",
    1,
)
s = s.replace("_processing ? 'PROCESSING…' : 'LOOP & SAVE'", "_processing ? 'PROCESSING…' : 'SAVE VIDEO'", 1)
p.write_text(s)


# Manual listing flow: first step is media with photos/video side by side.
p = Path("lib/src/features/add/presentation/screens/add_listing_screen.dart")
s = p.read_text()
pattern = r"class _PhotosStep extends ConsumerWidget \{.*?\n\}\n\nclass _DetailsStep extends ConsumerWidget \{"
match = re.search(pattern, s, flags=re.S)
if not match:
    raise SystemExit("manual media section not found")
manual_media = r'''class _PhotosStep extends ConsumerWidget {
...
class _DetailsStep extends ConsumerWidget {'''
s = s[: match.start()] + manual_media + s[match.end() :]

# Manual details start with description, then location and price/currency.
old_details_start = """        GlassTextField(
          controller: title,
...
"""
s = replace_once(s, old_details_start, """        GlassTextField(
          controller: description,
""", "manual description first")

old_after_description = """          maxLines: 5,
        ),
        GlassTextField(
          controller: country,
...
"""
new_after_description = """          maxLines: 5,
        ),
        const SizedBox(height: 8),
        Text(
        ...
        const SizedBox(height: 20),
"""
s = replace_once(s, old_after_description, new_after_description, "manual location price order")
p.write_text(s)
...
'''
marker = "  Future<void> _enhance() async {"
pos = s.find(marker)
if pos < 0:
    raise SystemExit("ai enhance marker missing")
...
""",
    "ai basics optional before extraction",
)

old_prompt = """        final structuredPrompt = '''
Currency: $_currency
City: $typedCity
Description:
$originalDescription
...
"""
new_extracted = """      final aiDescription = _parsedText(parsed, 'description');
      final description =
          aiDescription.isNotEmpty ? aiDescription : originalDescription;
      final country = _parsedText(parsed, 'country');
      final title = _parsedText(parsed, 'title');
...
"""
s = replace_once(s, old_extracted, new_extracted, "ai extracted basics")
s = replace_once(s, "          city: typedCity,", "          city: finalCity,", "final ai city")
...
# Replace category/basics/photos block with media-first + category + rent/sale.
start_marker = "                  const SizedBox(height: 18),\n                  _sectionTitle('WHAT ARE YOU LISTING?'),"
end_marker = "                  Row(\n                    children: [\n                      Expanded(child: _sectionTitle('DESCRIBE IT')),
"
start = s.find(start_marker)
end = s.find(end_marker)
if start < 0 or end < 0 or end <= start:
    raise SystemExit("ai top flow markers missing")
top_flow = r'''                  const SizedBox(height: 18),
                  ...
                  const SizedBox(height: 18),
'''
s = s[:start] + top_flow + s[end:]

# Add city/price/currency only after description + Enhance.
verification_marker = "                  const SizedBox(height: 18),\n                  _verificationCard(verificationDraft),"
basics = r'''                  const SizedBox(height: 18),
                  Row(
                    children: [
                  ...
                  const SizedBox(height: 18),
                  _verificationCard(verificationDraft),'''
s = replace_once(s, verification_marker, basics, "ai basics after description")

# Media/type widgets.
helper_marker = "  Widget _micStatusChip() {"
pos = s.find(helper_marker)
if pos < 0:
    raise SystemExit("ai helper marker missing")
helpers = r'''  Widget _mediaSection(int photoLimit) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Row(
...
# Sanity checks.
checks = {
    "video first": "media.insert(0, video);" in Path("lib/src/features/swipes/presentation/widgets/swipeable_card_stack.dart").read_text(),
    "range trim": "RangeSlider(" in Path("lib/src/features/camera/presentation/screens/video_cropper_screen.dart").read_text(),
    "manual media warning": "Keep media clean:" in Path("lib/src/features/add/presentation/screens/add_listing_screen.dart").read_text(),
    "ai media first": "_mediaSection(photoLimit)" in Path("lib/src/features/add/presentation/screens/ai_listing_builder_screen_v2.dart").read_text(),
    "ai autofill": "_fillBasicsFromDescription" in Path("lib/src/features/add/presentation/screens/ai_listing_builder_screen_v2.dart").read_text(),
    "ai video": "video: _video," in Path("lib/src/features/add/presentation/screens/ai_listing_builder_screen_v2.dart").read_text(),
}
failed = [name for name, ok in checks.items() if not ok]
if failed:
    raise SystemExit("sanity checks failed: " + ", ".join(failed))
print("listing media + AI flow patch ready")
