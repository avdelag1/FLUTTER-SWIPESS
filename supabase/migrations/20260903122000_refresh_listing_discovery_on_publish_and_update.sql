...
begin
  ...
  order by
    ...
    case when v_category not in ('recommended', 'popular')
      then greatest(coalesce(l.updated_at, l.created_at), l.created_at)
    end desc nulls last,
...
as $function$
  ...
  order by greatest(coalesce(l.updated_at, l.created_at), l.created_at) desc nulls last,
           l.created_at desc nulls last,
           l.id
