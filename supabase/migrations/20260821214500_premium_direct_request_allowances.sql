-- Keep App Store / Play receipt validators unchanged: their existing
-- user_subscriptions upsert is the single grant event. This trigger converts
-- the three live subscription products into finite Direct Request allowances.
...
BEGIN
  ...
  v_amount := CASE v_name
    WHEN 'Basic Client' THEN 6
    WHEN 'Premium Client' THEN 12
    WHEN 'Unlimited Client' THEN 30
    ELSE 0
  END;
