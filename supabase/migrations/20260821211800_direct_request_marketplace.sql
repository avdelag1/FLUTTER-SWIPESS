-- Swipess marketplace communication model
-- Interest is free. Mutual matches chat for free. Direct Requests reserve one
-- token and only consume it when the receiver accepts.
...
REVOKE ALL ON public.direct_requests FROM anon;
REVOKE INSERT, UPDATE, DELETE ON public.direct_requests FROM authenticated;
...
REVOKE ALL ON public.subscription_direct_request_grants FROM anon, authenticated;
...
BEGIN
  ...
  INSERT INTO public.notifications(
    ...
    CASE WHEN p_listing_id IS NULL
      ...
      ELSE 'Someone sent a priority request for your listing. Accept to open chat.'
    END,
    '/direct-request/' || v_request.id::text,
    v_uid,
    p_listing_id,
    jsonb_build_object(
...
BEGIN
  ...
  IF NOT p_accept THEN
    ...
    INSERT INTO public.notifications(
      ...
      'Your token was returned automatically.',
      v_uid,
...
CREATE OR REPLACE FUNCTION public.rpc_accept_listing_interest(
  p_liker_id uuid,
  p_listing_id uuid
)
RETURNS jsonb
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE x.user_id = p_liker_id
      AND x.target_id = p_listing_id
      AND x.target_type = 'listing'
      AND x.direction = 'right'
  ) THEN
    RAISE EXCEPTION 'This member has not expressed interest in the listing';
  END IF;
  ...
  IF v_conversation_id IS NULL THEN
    INSERT INTO public.conversations(
      ...
      v_match_id, p_liker_id, v_uid, p_listing_id, 'active', true
    )
    RETURNING id INTO v_conversation_id;
  ELSE
    UPDATE public.conversations
  ...
  INSERT INTO public.notifications(
    ...
    'new_match',
    'It’s a match',
    'Your interest was accepted. Chat is open for free.',
    '/messages/' || v_conversation_id::text,
    v_uid,
    p_listing_id,
    v_match_id,
...
BEGIN
  ...
  SELECT l.owner_id, COALESCE(NULLIF(trim(l.title), ''), 'your listing')
  ...
  INSERT INTO public.notifications(
    ...
    'new_like',
    'New interest',
    'Someone is interested in ' || v_title || '. Match them to open chat for free.',
...
-- Existing chat RPC is intentionally mutual-match only. Direct Requests use
-- their own accept path and never let Premium or tokens bypass consent.
...
BEGIN
  ...
  IF v_conversation_id IS NULL THEN
    ...
    INSERT INTO public.conversations(
      ...
      v_match.id, v_match.client_id, v_match.owner_id,
      COALESCE(p_listing_id, v_match.listing_id), 'active', true
    )
    RETURNING id INTO v_conversation_id;
    v_created := true;
  END IF;
...
-- Premium includes a finite number of Direct Requests so priority keeps value
-- and cannot become an unlimited spam channel.
...
BEGIN
  ...
  v_amount := CASE p_product_id
    WHEN 'Swipess.plus.monthly.v3' THEN 6
    WHEN 'swipess.plus.monthly.v2' THEN 6
    WHEN 'Swipess.plus.semestral.v3' THEN 12
    WHEN 'swipess.plus.semestral.v2' THEN 12
    WHEN 'Swipess.plus.annual.v3' THEN 30
    WHEN 'swipess.plus.annual.v2' THEN 30
    ELSE NULL
  END;
  ...
  INSERT INTO public.tokens(
    user_id, token_type, amount, total_activations, remaining_activations,
    used_activations, activation_type, source, notes, expires_at
  ) VALUES (
    p_user_id, 'messages', v_amount, v_amount, v_amount,
    0, 'subscription', 'premium_included',
    'Premium Direct Requests: ' || p_product_id, p_expires_at
  )
  RETURNING id INTO v_token_id;

  INSERT INTO public.subscription_direct_request_grants(
    transaction_key, user_id, product_id, amount, token_row_id
  ) VALUES (
    p_transaction_key, p_user_id, p_product_id, v_amount, v_token_id
  );
...
-- Harden SECURITY DEFINER entry points: only intended roles may invoke them.
...
GRANT EXECUTE ON FUNCTION public.rpc_get_direct_request_tokens() TO authenticated;
GRANT EXECUTE ON FUNCTION public.rpc_create_direct_request(uuid, uuid, text) TO authenticated;
GRANT EXECUTE ON FUNCTION public.rpc_respond_direct_request(uuid, boolean) TO authenticated;
GRANT EXECUTE ON FUNCTION public.rpc_cancel_direct_request(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.rpc_accept_listing_interest(uuid, uuid) TO authenticated;
