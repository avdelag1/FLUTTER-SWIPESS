...
set tags = (select array_agg(distinct tag order by tag) from unnest(coalesce(tags,'{}'::text[]) || array['jewlery','jewelery','jewlry','jewerly','jewlwey']::text[]) tag)
...
returns table (
  ...
  facebook text, tiktok text, youtube text, x_url text, telegram text, photo_url text,
  address text, neighborhood text, city text, region text, country text,
  latitude double precision, longitude double precision, service_radius_km double precision,
  hours text, languages text[], price_level text, tags text[], recommendation_note text,
...
begin
  ...
  with base as (
    select e.*,
      ...
      case when p_lat is not null and p_lon is not null and e.latitude is not null and e.longitude is not null then
        6371.0 * acos(least(1.0,greatest(-1.0,
          cos(radians(p_lat))*cos(radians(e.latitude))*cos(radians(e.longitude)-radians(p_lon)) + sin(radians(p_lat))*sin(radians(e.latitude))
        ))) else null end as calculated_distance_km,
      case
        when nullif(trim(coalesce(p_city,'')),'') is not null and (lower(p_city) like '%'||lower(e.city)||'%' or lower(e.city) like '%'||lower(p_city)||'%') then 2
        when lower(e.city)='global' then 1
        else 0
    ...
    from regexp_split_to_table(extensions.unaccent(lower(coalesce(p_query,''))), E'\\s+') token
  ...
  select s.id,s.entry_type,s.name,s.category,s.description,s.phone,s.whatsapp,s.email,s.website,s.instagram,
    s.facebook,s.tiktok,s.youtube,s.x_url,s.telegram,s.photo_url,s.address,s.neighborhood,s.city,s.region,s.country,
    s.latitude,s.longitude,s.service_radius_km,s.hours,s.languages,s.price_level,s.tags,s.recommendation_note,
    s.is_featured,s.is_verified,s.priority,s.linked_profile_user_id,s.linked_listing_id,
  ...
  left join lateral (
    ...
    where s.linked_listing_id is not null and l.id=s.linked_listing_id and l.is_active=true and l.status='active' limit 1
  ...
  order by s.location_rank desc,s.exact_name_in_query desc,s.exact_token_matches desc,s.edit_distance asc,s.fuzzy_score desc,
    s.calculated_distance_km asc nulls last,s.is_featured desc,s.is_verified desc,s.priority desc,s.updated_at desc
