-- Improve AI Local Brain relevance.
--
-- Key fixes:
-- 1) match whole words instead of substrings (so "man" no longer matches
--    unrelated words such as "management"),
-- 2) remove the selected discovery city from semantic intent tokens, and
-- 3) rank semantic token coverage before location proximity.
--
-- This keeps location useful as a tie-breaker without allowing "Tulum" alone
-- to make unrelated local businesses outrank a strongly matching curated VIP.
