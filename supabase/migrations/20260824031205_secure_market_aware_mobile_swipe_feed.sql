-- Authenticated Flutter discovery feed. The backend derives the caller from
-- auth.uid(), applies the selected Passport market, and removes categories that
-- Super Admin disabled for that city/country. Recommended and Popular aggregate
-- only across categories that remain enabled.
...
begin
  ...
  where coalesce(l.is_active, true) = true
    ...
    and public._discovery_listing_visible(
      v_uid, l.id, l.price, l.hourly_rate, l.description, l.images
    )
  order by
    ...
    case when v_category = 'recommended' and coalesce(l.has_verified_documents, false) then 1 else 0 end desc,
