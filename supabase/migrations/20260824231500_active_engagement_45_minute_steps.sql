-- Active-use reward ladder: five 45-minute engagement steps unlock one token.
-- Existing accumulated active seconds are preserved; only the next threshold changes.
-- The exact cadence is intentionally kept out of user-facing UI copy.
...
DECLARE
  ...
  v_points int := 0;
  v_step_seconds constant int := 2700;
BEGIN
...
BEGIN
  ...
  IF v_tokens_earned > 0 THEN
    ...
    INSERT INTO public.notifications (
      ...
      'system_announcement',
      'Free token unlocked',
      'You completed your activity reward and earned a free token.',
      '/client/profile',
      jsonb_build_object(
  ...
  ELSIF v_steps_earned > 0 THEN
    INSERT INTO public.notifications (
      ...
      format('You reached step %s/5 toward your next reward.', v_new_steps),
      '/client/profile',
      jsonb_build_object(
