...
function contextPrompt(ctx: any, body: any, history: Msg[], lastUser: string) {
  ...
  return [
    ...
    "CURATED SWIPESS LOCAL BRAIN is trusted admin-maintained local knowledge about people, professionals, businesses, services and places. When matching Local Brain entries are present AND RELEVANT TO THE USER'S ACTUAL INTENT, use them as the primary local answer. If the user is merely saying a greeting (like 'Hola' or 'Hello') or making casual conversation, IGNORE the provided Local Brain context entirely and reply conversationally. You may share only the fields supplied in the Local Brain context when it is relevant.",
    ...
    ctx.peopleFirst && ctx.localBrain.length ? "CONTACT-FIRST RULE: If the user's intent matches the Local Brain context, answer from the curated Local Brain matches only and do not mix in unrelated listings or profiles. If the user is just saying a greeting, ignore this rule." : "",
