...
async function requireAccess(req: Request) {
  ...
  if (entitlementError) {
    console.error("[ai-concierge-v74] entitlement", entitlementError.message);
...
async function loadContext(client: any, query: string) {
  ...
  if (category) {
    try {
      ...
      console.error("[ai-concierge-v74] listings context", String(e));
    }
  }
  ...
  if (wantsEvents(query)) {
    try {
      ...
      console.error("[ai-concierge-v74] events context", String(e));
    }
  }
  ...
  if (wantsPeople(query)) {
    try {
      ...
      console.error("[ai-concierge-v74] profiles context", String(e));
    }
  }
...
function contextPrompt(ctx: any, body: any) {
  const location = body?.locationContext?.passportLabel?.toString().trim();
  const character = body?.character?.toString().trim();
  return [
    "You are SWIPESS AI, the central concierge for the SWIPESS marketplace.",
    "Reply in the same language as the user's latest message.",
    "Be concise, useful, conversational and action-oriented.",
    "Never invent listing IDs, prices, users, events, phone numbers or completed actions.",
    ...
    "Useful categories: properties, workers/services, yachts, motorcycles, bicycles, events, people/seekers, legal, documents.",
    location ? `Current discovery location: ${location}.` : "",
    character ? `Requested persona: ${character}. Keep that tone while staying accurate.` : "",
    ctx.listings.length ? `LIVE LISTINGS:\n${JSON.stringify(ctx.listings)}` : "",
    ctx.events.length ? `LIVE EVENTS:\n${JSON.stringify(ctx.events)}` : "",
    ctx.profiles.length ? `LIVE PEOPLE:\n${JSON.stringify(ctx.profiles)}` : "",
  ].filter(Boolean).join("\n\n");
}
...
async function groq(messages: Msg[]) {
  ...
  const res = await fetch("https://api.groq.com/openai/v1/chat/completions", {
    ...
    body: JSON.stringify({ model: "llama-3.3-70b-versatile", messages, max_tokens: 900, temperature: 0.45, stream: false }),
  });
...
async function gemini(system: string, history: Msg[]) {
...
async function kimi(messages: Msg[]) {
  ...
  const res = await fetch("https://api.moonshot.cn/v1/chat/completions", {
    ...
    body: JSON.stringify({ model: "moonshot-v1-8k", messages, max_tokens: 900, temperature: 0.4, stream: false }),
  });
  if (!res.ok) throw new Error(`http ${res.status}`);
  const text = openAiText(await res.json());
  if (!text) throw new Error("empty");
...
async function minimax(messages: Msg[]) {
  ...
  const res = await fetch("https://api.minimaxi.chat/v1/text/chatcompletion_v2", {
    ...
    body: JSON.stringify({ model: "MiniMax-M2.7", messages, max_tokens: 900, temperature: 0.4, stream: false }),
  });
  if (!res.ok) throw new Error(`http ${res.status}`);
  const data = await res.json();
...
function emergencyReply(query: string, ctx: any) {
  ...
  if (/^\s*(hi|hey|hello|hola|buenas|yo|sup)\b/i.test(query)) {
    return "Hey — I’m here. What do you want to find: properties, workers, events, yachts, motorcycles, bicycles, people, legal help, or something else?";
  }
  return "I’m here and SWIPESS search is online. Tell me what you want to find — properties, workers, events, yachts, motorcycles, bicycles, people, legal help, or documents — and I’ll take you there.";
}

Deno.serve(async (req) => {
  if (req.method === "OPTIONS") return new Response("ok", { headers: corsHeaders });
  if (req.method === "GET") return json(200, { status: "ready", service: "ai-concierge", mode: "resilient-json" }, "swipess", "health");
  ...
  try {
    ...
    const system = contextPrompt(ctx, body);
    ...
    const attempts: Array<[string, () => Promise<string>]> = [
      ...
      ["gemini", () => timeout(gemini(system, history))],
    ...
    for (const [provider, run] of attempts) {
      try {
        ...
        return json(200, { choices: [{ message: { content: text } }] }, provider, "provider-json");
      } catch (e) {
        const message = e instanceof Error ? e.message : String(e);
        errors.push(`${provider}:${message}`);
        console.error(`[ai-concierge-v74] ${provider} failed`, message);
      }
    }

    console.error("[ai-concierge-v74] all providers failed", errors.join(" | "));
    const local = emergencyReply(lastUser, ctx);
    return json(200, { choices: [{ message: { content: local } }] }, "swipess-local", "emergency-local");
  } catch (e) {
    console.error("[ai-concierge-v74] fatal", e instanceof Error ? e.message : String(e));
    return json(200, {
      choices: [{ message: { content: "I’m here. Tell me what you want to find on SWIPESS — properties, workers, events, yachts, motorcycles, bicycles, people, legal help, or documents." } }],
