...
async function requirePremium(req: Request): Promise<Response | null> {
  ...
  if (!authorization || !SUPABASE_URL || !SUPABASE_ANON_KEY) return json(401, { error: "Sign in required." });
  ...
  try {
    ...
    async function callGroq(systemContent: string, userContent: string, jsonMode = true) {
      ...
      if (!res.ok) {
        ...
        throw new Error(res.status === 429 ? "Rate limit" : `Groq ${res.status}: ${t}`);
      }
...
Detect exactly one category: property, motorcycle, bicycle, yacht, worker. The user pre-selected "${hintCategory}"; only override if their words clearly describe another category. Never invent facts. Use null for missing values.
...
Base price: ${body.price ?? "(unknown)"}. If it is a valid positive number, preserve it exactly.
...
Return ONLY valid JSON with all keys:
{
 ...
 "title":string,"description":string,"price":number|null,"city":string|null,
 "beds":number|null,"baths":number|null,"square_footage":number|null,"property_type":string|null,"furnished":boolean|null,"pet_friendly":boolean|null,
 "year":number|null,"make":string|null,"model":string|null,"engine_cc":number|null,"mileage":number|null,"motorcycle_type":string|null,"bicycle_type":string|null,"transmission":string|null,"fuel_type":string|null,"condition":string|null,
 "yacht_type":string|null,"length_m":number|null,"berths":number|null,"max_passengers":number|null,
 "service_category":string|null,"pricing_unit":string|null,"experience_years":number|null,"skills":[],"amenities":[]
}`;
    ...
    const validCategories = ["property", "motorcycle", "bicycle", "yacht", "worker"];
    if (!validCategories.includes(parsed.category as string)) parsed.category = validCategories.includes(hintCategory) ? hintCategory : "property";
    const userPrice = Number(body.price);
    if (Number.isFinite(userPrice) && userPrice > 0) parsed.price = userPrice;
